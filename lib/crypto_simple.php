...
Trait Crypto_simple {
    ...
    public static function crypto_crypt(string $plaintext){
        ...
        $key = openssl_random_pseudo_bytes(self::$key_size, true);
        $ivlen = openssl_cipher_iv_length(self::$cipher);
        $iv = openssl_random_pseudo_bytes($ivlen);
        $ciphertext = openssl_encrypt($plaintext, self::$cipher, $key, $options=0, $iv, $tag);
    ...
    public static function crypto_uncrypt($cipher_back, string $cipher_key){
        ...
        return openssl_decrypt($cipher_back->ciphertext, self::$cipher, $cipher_key, $options=0, $cipher_back->iv, $cipher_back->tag);
    }    
}